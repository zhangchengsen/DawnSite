<template>
  ...
  >
    ...
    <n-form-item v-if="type !== 'login'" :show-label="false" path="repassword">
      <n-input v-model:value="form.repassword" placeholder="确认密码" />
    </n-form-item>
    <div class="flex justify-between w-full mb-2">
      <NButton quaternary type="primary" @click="changeType">{{
        type !== "login" ? "登录" : "注册"
      }}</NButton>
      <NButton quaternary type="primary" class="text-gray-500" size="tiny"
        >忘记密码?</NButton
      >
    </div>
    <div>
...
function onSubmit() {
  formRef.value.validate(async (errors) => {
    ...
    console.log("验证通过");
    loading.value = true;
    let { data, error, pending } = await useLoginApi(form);
    loading.value = false;
